...
file_path = '/kaggle/input/stockdb/*.csv'  # 修改为解压后的数据集路径
...
# Fill missing values
df[features] = df[features].ffill()  # 使用前一天的数据填充缺失值
...
def predict_stock_value(stock_code, date):
    ...
    new_data = pd.DataFrame(new_data, columns=features).ffill().values  # 填充缺失值
